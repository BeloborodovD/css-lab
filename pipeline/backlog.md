...
- [ ] В темах `veza.css` и `hemah.css` блок `@media (prefers-color-scheme: dark)`
      зеркалит только акцентный ряд, а `--color-info*` и `--brand-secondary*`
      остаются светлыми: они объявлены на `[data-brand]` (уровень body) и бьют
      тёмную базу с `:root`. При системной тёмной теме без класса `.dark`
      `--color-info` даёт 2.98:1 у VEZA и около того у HEMAH — ниже AA.
      В `uralelectro.css` это закрыто при переводе на канон (ORD-01), два
      других бренда — чужая зона этой волны. (Найдено: шаг 03, ORD-01.)
